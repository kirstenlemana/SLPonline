...
?>
  ...
  <div class="row" style="padding-left:1em;padding-right:0.5em"> 
    <div style="border:solid 1px #c5d6de;margin-left:1em;background:#fff;padding:1em;padding-left:1.5em;padding-right:1.5em;font-size:12px">
     <?php 
    $stmt=$db->prepare("SELECT w.wall_msg,h.firstname,h.sex,w.wallposted,p.name,h.lastname,h.id,w.wallowner,h1.firstname,h1.lastname FROM wallposts as w LEFT JOIN HRDB as h on w.wallposter=h.id LEFT JOIN hr_profilepics as p on p.hrdbid=h.id LEFT JOIN HRDB as h1 on h1.id=w.wallowner ORDER BY w.wallpostid DESC LIMIT 0,10");
    $stmt->execute();
